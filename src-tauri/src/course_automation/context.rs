...
出力方針:\n\
- summary: この資料が学生にとって何を意味するかを1文、原則120文字以内。授業名・大学名・資料名の復唱、本文の言い換え、背景説明は不要です。\n\
- findings: 学生が行動・注意・判断に使える具体的事実だけを最大3件。期限、変更点、提出条件などを優先し、重複させないでください。\n\
- seatEvidence: 座席・教室・学籍番号との対応を判断できる根拠だけ。座席と無関係なら空配列にしてください。\n\
- printInstruction: 印刷、手書き、紙提出、持参が明示された場合だけ、必要な行動を短く記録してください。推測は禁止です。\n\
- triggerDecision: 期限が近い、既存判断を変える、即時対応が必要なら immediate。今後の新情報との照合が必要なら observe。それ以外は routine。単語一致ではなく影響で判断してください。\n\
...
- 座席情報は seat にだけ記録し、summary/findings へ出力してはいけません。assignment は「7列目」「指定なし」のような現在の結論だけを短く示し、根拠は evidence に分離してください。\n\
