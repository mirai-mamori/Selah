...
- findings: 学生が今・近いうちに気に留めるべき一時的で具体的な事項を {\"text\":\"...\",\"expiresAt\":\"YYYY-MM-DD\",\"action\":false} のオブジェクト配列で記録します。ここには次の2種類が入ります。(A)学生が実際に手を動かす必要がある行動(提出・準備・申込・出席・印刷して記入など)。これは action:true にします。(B)行動は不要だが今・近いうちに知っておくべき通知・変更・連絡(教室や時間の変更、休講・補講、次回小テストの範囲、配布や公開の予定、訂正のお知らせなど)。これは action:false にします。両方とも findings に含めてよく、(A)を優先して上位に並べます。ただし、学期を通じてずっと当てはまる恒常的なルール・前提・背景・使用教材・一般的な運用方針は findings ではなく standingContext に入れてください(例:「発表・レポートでAI使用禁止」「教科書は○○を使用中」「締切に誤りが出たら訂正掲示で再設定される運用」は standingContext 行き)。振り分けの基準は、findings=『一時的で、いずれ古くなる/不要になる情報』、standingContext=『恒常的に成り立つ前提』です。該当がなければ空配列 [] にし、無理に作らないこと。件数の上限はありません。各 text は単独で理解できる短い1文とし、summary と重複させないでください。expiresAt にはその項目が無意味になる日付(締切日・対象授業回の日付・イベント当日など)を YYYY-MM-DD で入れ、過ぎたものは出力しないでください。特定の失効日がなければ空文字 \"\" にします。action:true の項目だけが学生の TODO リストへ自動追加されます。通知・連絡・お知らせなど『知っておくだけ』のものは、たとえ重要でも必ず action:false にし、TODO 化させないでください。\n\
